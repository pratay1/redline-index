...
const selfHostedImagePath = z.string().regex(/^\/catalog-images\/[a-z0-9][a-z0-9._-]*\.(?:avif|gif|jpe?g|png|webp)$/i, "Use a self-hosted path under /catalog-images/.");
...
export const imageSchema = z.object({ vehicleId: idSchema, sourceId: z.preprocess((value) => value === "" ? undefined : value, idSchema.optional()), url: selfHostedImagePath, alt: z.string().trim().min(3).max(300), credit: optionalText, position: z.coerce.number().int().min(0).max(50) });
...
export const trimSchema = z.object({
  ...
  imageUrl: z.preprocess((value) => value === "" ? undefined : value, selfHostedImagePath.optional()), imageAlt: optionalText, imageCredit: optionalText, imageSourceId: z.preprocess((value) => value === "" ? undefined : value, idSchema.optional()), sourceId: z.preprocess((value) => value === "" ? undefined : value, idSchema.optional()),
});
