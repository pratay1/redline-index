/**
 * Shared Ferrari seed helpers and reserved exterior image URLs.
 * Prefer Ferrari media / press exteriors; auto-data.net OK.
...
export const RESERVED_FERRARI_IMAGE_URLS = new Set<string>([]);

/**
 * US Ferrari destination & handling (cents).
 * iSeeCars Monroney-style destination for MY2024–2025 296 GTB: $5,000.
 * Prefer per-trim Monroney / Ferrari of North America footnotes when they differ.
 * Older MotorTrend note: 812 GTS carried ~$3,950 — use trim-specific when cited.
 */
export const FERRARI_DESTINATION_CENTS = 500_000;
...
export async function assertImageUrlOk(url: string) {
  if (RESERVED_FERRARI_IMAGE_URLS.has(url)) {
  ...
  RESERVED_FERRARI_IMAGE_URLS.add(url);
  return url;
}
...
export async function ensureFerrariManufacturer(prisma: PrismaClient) {
...
export type FerrariEngineInput = {
 ...
 */
export async function ensureFerrariEngine(
  prisma: PrismaClient,
  data: FerrariEngineInput,
) {
  if (data.code) {
