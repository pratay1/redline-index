/**
 * Shared Porsche seed helpers and reserved exterior image URLs.
...
export const RESERVED_PORSCHE_IMAGE_URLS = new Set<string>([]);

/** US Porsche destination & handling (cents). MY2024–2025 press: $1,995. */
export const PORSCHE_DESTINATION_CENTS = 199500;
...
export async function assertImageUrlOk(url: string) {
  if (RESERVED_PORSCHE_IMAGE_URLS.has(url)) {
  ...
  RESERVED_PORSCHE_IMAGE_URLS.add(url);
  return url;
}
...
export async function ensurePorscheManufacturer(prisma: PrismaClient) {
...
export type PorscheEngineInput = {
 ...
 */
export async function ensurePorscheEngine(
  prisma: PrismaClient,
  data: PorscheEngineInput,
) {
  if (data.code) {
